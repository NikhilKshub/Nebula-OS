...
// wrapping these so crashes on private browsing don't kill everything
...
// opens the notes window with a welcome message on first load
...
function updateClock() {
  ...
  if (clockEl) clockEl.textContent = `${h}${colonOn ? ':' : ' '}${m}`;
  if (dateEl) dateEl.textContent = now.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }).toUpperCase();
}
...
function updateTelemetry() {
  ...
  if (intEl) {
    intEl.textContent = sysIntegrity.toFixed(2) + '%';
    intEl.style.color = sysIntegrity < 50 ? 'var(--red)' : sysIntegrity < 80 ? 'var(--yellow)' : 'var(--green)';
  }
}
...
function fakeReboot() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      const status = document.querySelector('.boot-status');
      if (boot) {
        Object.assign(boot.style, { display: 'flex', opacity: '1', transform: 'none', transition: 'none' });
...
function doWormhole() {
  ...
  // safety net — if rAF gets throttled, force reload after 12s
...
function initPaint() {
  ...
  // mouseup on document is also handled globally below
...
// global mouseup so releasing outside canvas still stops drawing
...
function startGame() {
  ...
  const canvas = document.getElementById('game-canvas');
  if (!canvas) return;
  const grid = 15, tiles = Math.floor(canvas.width / grid);
